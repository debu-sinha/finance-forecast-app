...
def _setup_mlflow_fixture():
    """Ensure MLflow uses local file store, not Databricks tracking URI.

    Without this, importing backend.main in other tests sets
    mlflow.set_tracking_uri("databricks") globally, causing all model
    logging tests to fail with RESOURCE_DOES_NOT_EXIST.

    Uses module scope so the tracking directory persists across all tests
    in this file (each test's artifacts need to survive for load_model).
    """
    tracking_uri = "file:///tmp/mlruns_test_models"
    if os.path.exists("/tmp/mlruns_test_models"):
        shutil.rmtree("/tmp/mlruns_test_models")
    os.makedirs("/tmp/mlruns_test_models", exist_ok=True)
    mlflow.set_tracking_uri(tracking_uri)
    mlflow.set_experiment("test_experiment")
    os.environ["MLFLOW_MAX_WORKERS"] = "1"
    yield
    # Restore default so we don't leak state to other test modules
    mlflow.set_tracking_uri("")


def generate_synthetic_data(n_rows=100, freq='D'):
    """Generate synthetic time series data with clear trend + seasonality.

    Uses a positive baseline with linear trend and sinusoidal seasonality
    so models like ARIMA can detect non-flat structure.
    """
    np.random.seed(42)
    dates = pd.date_range(start='2024-01-01', periods=n_rows, freq=freq)
    t = np.arange(n_rows)
    # Positive baseline (100) + upward trend + strong seasonality + noise
    y = 100 + t * 0.5 + 20 * np.sin(2 * np.pi * t / 30) + np.random.normal(0, 3, n_rows)
